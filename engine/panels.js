export class PanelManager{
  constructor(root){ this.root = root; this.registry = new Map(); this.instances = new Map(); }
  ...
  toggle(id){ const inst = this.ensure(id); if (!inst) return; inst.open ? this.close(id) : this.open(id); }
  open(id){
    for (const key of this.instances.keys()) if (key !== id) this.close(key);
    const inst = this.ensure(id);
    inst.open = true; inst.el.classList.add('open'); this.render(id);
  }
  close(id){ const inst = this.instances.get(id); if (!inst) return; inst.open = false; inst.el.classList.remove('open'); }
  ...
  render(id){ const inst = this.instances.get(id); if (!inst) return; inst.def.render(inst.body); }
